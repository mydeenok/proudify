@php
    $newUploadUrl = $isAdmin ? route('admin.bulk-upload.create') : route('bulk-upload.select-template');
@endphp
...
<div class="flex flex-col md:flex-row md:items-end justify-between gap-md mb-xl">
    ...
    <a href="{{ $newUploadUrl }}" class="btn-primary h-11 shrink-0">
...
<form method="GET" class="bg-surface-container-lowest border border-outline-variant rounded-xl p-md shadow-sm flex flex-wrap gap-md items-center mb-gutter">
    <x-filter-select name="status" class="min-w-[180px]">
        <option value="">All Statuses</option>
        <option value="mapping" @selected(request('status') === 'mapping')>Mapping columns</option>
        <option value="processing" @selected(request('status') === 'processing')>Processing</option>
        <option value="completed" @selected(request('status') === 'completed')>Completed</option>
        <option value="completed_with_errors" @selected(request('status') === 'completed_with_errors')>Completed with errors</option>
        <option value="failed" @selected(request('status') === 'failed')>Failed</option>
    </x-filter-select>

    @if ($isAdmin)
        <x-filter-select name="user_id" class="min-w-[200px]">
            <option value="">All Organizations</option>
            @foreach ($orgUsers as $orgUser)
                <option value="{{ $orgUser->id }}" @selected((string) request('user_id') === (string) $orgUser->id)>{{ $orgUser->organization_name }}</option>
            @endforeach
        </x-filter-select>
    @endif
        ...
        <table class="w-full text-left border-collapse min-w-[900px]">
            ...
            <tbody class="divide-y divide-outline-variant">
                @forelse ($batches as $batch)
                    <tr class="h-16 hover:bg-surface-container transition-colors">
                        ...
                        <td class="px-md py-xs text-right">
                            <div class="flex items-center justify-end gap-xs">
                                @if ($batch->status === 'mapping')
                                    <a href="{{ route('bulk-upload.map-columns', $batch) }}" class="p-1 rounded hover:bg-surface-variant text-on-surface-variant hover:text-primary transition-colors" title="Continue: map columns">
                                        <span class="material-symbols-outlined text-[20px]">arrow_forward</span>
                                    </a>
                                @elseif ($batch->status === 'queued')
                                    <a href="{{ route('bulk-upload.review', $batch) }}" class="p-1 rounded hover:bg-surface-variant text-on-surface-variant hover:text-primary transition-colors" title="Continue: review and issue">
                                        <span class="material-symbols-outlined text-[20px]">arrow_forward</span>
                                    </a>
                                @else
                                    <a href="{{ route('bulk-upload.status', $batch) }}" class="p-1 rounded hover:bg-surface-variant text-on-surface-variant hover:text-primary transition-colors" title="View status">
                    ...
                    <tr>
                        <td colspan="{{ $isAdmin ? 6 : 5 }}" class="px-md py-2xl text-center">
                            <div class="flex flex-col items-center gap-sm text-on-surface-variant">
                                ...
                                <p class="font-body-md text-body-md">No bulk uploads {{ request()->hasAny(['status', 'user_id']) ? 'match these filters' : 'yet' }}.</p>
                                @unless (request()->hasAny(['status', 'user_id']))
                                    <a href="{{ $newUploadUrl }}" class="font-label-md text-label-md text-primary hover:underline">Start your first bulk upload</a>
                                @endunless
                            </div>
                        </td>
                    </tr>
