<x-layouts.user-shell title="Checkout">
    <div class="max-w-[960px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-gutter">
        <div>
            <x-page-header
                ...
                description="Issuing {{ $order->quantity }} certificates from &quot;{{ $order->template->name }}&quot;."
            />
            ...
            <div class="card-surface p-lg shadow-card-sm space-y-md">
                <div class="flex items-center gap-md pb-md border-b border-outline-variant">
                    ...
                    <div>
                        ...
                        <p class="font-body-sm text-body-sm text-on-surface-variant">{{ $order->quantity }} certificates</p>
                    </div>
                </div>
    ...
    <script>
        ...
        payButton.addEventListener('click', async function () {
            ...
            let order;
            try {
                const orderResponse = await fetch('{{ route('bulk-upload.checkout.create-order', $order) }}', {
                    method: 'POST',
                    headers: {
            ...
            const razorpay = new Razorpay({
                ...
                description: '{{ $order->template->name }} ({{ $order->quantity }} certificates)',
                theme: { color: '#d92727' },
                prefill: {
                    name: '{{ auth()->user()->name }}',
                ...
                handler: async function (response) {
                    ...
                    try {
                        const verifyResponse = await fetch('{{ route('bulk-upload.checkout.verify-payment', $order) }}', {
                            method: 'POST',
                            headers: {
