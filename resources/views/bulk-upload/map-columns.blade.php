...
<x-layouts.contextual-shell title="Map Columns">
    <div class="max-w-3xl mx-auto">
        ...
        <form method="POST" action="{{ route('bulk-upload.map-columns.store', $batch) }}" class="card-surface p-lg bento-shadow-sm space-y-md">
                ...
                <div>
                    ...
                    <select id="{{ $field }}" name="mapping[{{ $field }}]" @required(in_array($field, $required))
                        ...
                        @foreach ($headers as $index => $header)
                            <option value="{{ $index }}" @selected(old("mapping.$field") == $index)>{{ $header ?: 'Column '.($index + 1) }}</option>
                        @endforeach
                    </select>
            ...
            <div class="pt-md flex justify-between items-center border-t border-outline-variant">
                <a href="{{ auth()->user()->isAdmin() ? route('admin.bulk-upload.create') : route('bulk-upload.create', ['template' => $batch->template_id]) }}" class="btn-secondary">Back</a>
