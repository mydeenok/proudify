<x-layouts.guest title="Log in">
        ...
        <div class="flex flex-col gap-base">
            <div class="flex justify-between items-center gap-sm">
                ...
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}" class="font-label-sm text-label-sm text-secondary hover:text-primary hover:underline transition-colors">Forgot password?</a>
                @endif
            </div>
    ...
    <div class="flex items-center gap-md">
        ...
        <span class="font-label-sm text-label-sm uppercase tracking-wider text-on-surface-variant">Or</span>
        <div class="h-px bg-outline-variant flex-1"></div>
    </div>

    <div class="grid grid-cols-2 gap-sm">
        <a href="{{ route('sso.redirect', 'Google') }}" class="h-11 flex items-center justify-center gap-sm border border-outline-variant bg-surface hover:bg-surface-container-low hover:border-outline text-on-surface font-label-md text-label-md rounded-lg transition-colors">
        ...
        <a href="{{ route('sso.redirect', 'Facebook') }}" class="h-11 flex items-center justify-center gap-sm border border-outline-variant bg-surface hover:bg-surface-container-low hover:border-outline text-on-surface font-label-md text-label-md rounded-lg transition-colors">
            <svg class="w-5 h-5 shrink-0" viewBox="0 0 24 24" aria-hidden="true"><path fill="#1877F2" d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z"/></svg>
            Facebook
        </a>
    </div>
