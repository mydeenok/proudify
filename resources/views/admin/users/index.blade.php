@php
    $pendingApprovalCount = \App\Models\User::where('status', 'pending_approval')->count();
@endphp
<x-layouts.admin-shell title="Users">
    ...
    <div class="card-surface bento-shadow-sm overflow-hidden">
        <div class="flex flex-col md:flex-row justify-between items-center border-b border-outline-variant px-md">
            ...
            <form method="GET" class="flex flex-wrap items-center gap-sm py-sm w-full md:w-auto">
                ...
                <select name="status" class="h-10 rounded-lg border border-outline-variant bg-surface px-sm font-body-sm text-body-sm text-on-surface focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none">
                    <option value="">All statuses</option>
                    <option value="active" @selected(request('status') === 'active')>Active</option>
                    <option value="pending_otp" @selected(request('status') === 'pending_otp')>Pending OTP</option>
                ...
                <select name="role" class="h-10 rounded-lg border border-outline-variant bg-surface px-sm font-body-sm text-body-sm text-on-surface focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none">
                    <option value="">All roles</option>
                    <option value="admin" @selected(request('role') === 'admin')>Admin</option>
                    <option value="user" @selected(request('role') === 'user')>User</option>
                </select>

                <button type="submit" class="h-10 px-sm bg-surface border border-outline-variant rounded-lg text-on-surface-variant hover:bg-surface-variant transition-colors flex items-center justify-center" title="Apply filters">
